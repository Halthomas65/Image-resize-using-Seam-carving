...
__global__ void calcMinValAndKeepTrackKernel(uint32_t* minValMatrix, int width, int r, int* trackCol){
	int c = blockIdx.x * blockDim.x + threadIdx.x;
	if (c < width){
...
void seamCarvingByDevice2(uchar3 * inPixels, int width, int height, int nSeams, uchar3*& outPixels){
	...
	while (width > expectWidth) {
		...
		int* d_trackCol;
		CHECK(cudaMalloc(&d_trackCol, width * (height - 1) * sizeof(int)));
		dim3 blockSizeMinVal(BLOCK_SIZE);
		dim3 gridSizeMinVal((width - 1) / blockSizeMinVal.x + 1);
		for (int r = height - 2; r >= 0; r--){
			calcMinValAndKeepTrackKernel<<<gridSizeMinVal, blockSizeMinVal>>>(d_importanceMatrix, width, r, d_trackCol);
		}
		...
		CHECK(cudaMemcpy(trackCol, d_trackCol, width * (height - 1) * sizeof(int), cudaMemcpyDeviceToHost));
...
int main(int argc, char ** argv)
{
	...
	if (nSeams >= width || nSeams < 0){
		printf("So seam nhap vao khong hop le");
		return 0;
	}
	...
	GpuTimer timer;
	timer.Start();
	seamCarvingByDevice2(inPixels, width, height, nSeams, outPixels);
	timer.Stop();
	float time = timer.Elapsed();
	printf("Device version 2 - Number of seams = %i - Kernel time = %f ms\n", nSeams, time);
	char * outFileNameBase = strtok(argv[2], ".");
	writePnm(outPixels, width - nSeams, height, concatStr(outFileNameBase, "_device2.pnm"));
	free(inPixels);
	free(outPixels);
}
